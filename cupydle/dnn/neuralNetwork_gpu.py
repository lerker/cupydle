...
def train(trainSet, validSet, testSet,
    ...
    """
    train_img,  train_labels= shared_dataset(trainSet)
    test_img,   test_labels = shared_dataset(validSet)
    val_img,    val_labels  = shared_dataset(testSet)

    # compute number of minibatches for training, validation and testing
    n_train_batches = train_img.get_value(borrow=True).shape[0] // batch_size
    n_valid_batches = val_img.get_value(borrow=True).shape[0] // batch_size
    n_test_batches  = test_img.get_value(borrow=True).shape[0] // batch_size
    ...
    test_model = theano.function(
        ...
        givens={
            x: test_img[index * batch_size:(index + 1) * batch_size],
    ...
    validate_model = theano.function(
        ...
        outputs=classifier.errors(y),
        givens={
            x: val_img[index * batch_size:(index + 1) * batch_size],
            y: val_labels[index * batch_size:(index + 1) * batch_size]
        }
    )
    ...
    train_model = theano.function(
        ...
        updates=updates,
        givens={
            x: train_img[index * batch_size: (index + 1) * batch_size],
...
if __name__ == '__main__':
    ...
    #MNIST.plot_one_digit(train_img.get_value()[0])
    train(  trainSet=mnData.get_training(),
